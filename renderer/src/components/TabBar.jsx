...
const TabBar = memo(function TabBar() {
    ...
    return (
        <div
            className={`flex items-center gap-2 px-4 py-2.5 overflow-x-auto rounded-[28px] mx-1 ${theme === 'dark' ? 'bg-white/5 border border-white/10' : 'bg-black/5 border border-black/10'
                }`}
        >
            {tabs.map((tab, index) => (
                <div
                    ...
                    className={`group flex items-center gap-2 px-4 py-2 rounded-full cursor-grab active:cursor-grabbing transition-all min-w-[100px] max-w-[180px] ${draggedIndex === index ? 'opacity-50 scale-95' : ''
                        } ${dragOverIndex === index ? 'ring-2 ring-white/30' : ''
                        } ${activeTabId === tab.id
